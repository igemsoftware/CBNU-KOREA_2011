...
public class CircularMapV70 extends JComponent {
...
*/	
	...
	public CircularMapV70(String viewOption, int seqLen, int genomeOption, MonitorPanel monitor, String orgName) {
...
*/
	...
	public void drawGenes(Graphics2D g2) {
		...
		generalpath = new GeneralPath[getGeneNumber()];
		double a = 2 * Math.PI / seqLen;

		for (int k = 0; k < getGeneNumber(); k++) {
			double geneStartPoint = (double)(getSP(k));
			double geneEndPoint = (double)(getEP(k));
			
			if (getStrand(k)== -1) {                   //안쪽에 gene을 표시할 다각형의 각 좌표를 설정한다.
...
//		float divideFactor = seqLen/2880000; //기본단위 설정의 기준 유전체 길이 2880000 bp (240*2pi*2*1000)
	...
	private int getSP(int i) {                        // 유전자 시작 위치 반환
		return (spVec.elementAt(i).intValue());
	}
	
	private int getEP(int i) {                        // 유전자 끝위치 반환
		return (epVec.elementAt(i).intValue());
	}
	
	private int getGeneNumber() {                             // 유전자 개수 반환
		return spVec.size();
	}
	private int getStrand(int i) {                     // 유전자 방향      +1, -1
