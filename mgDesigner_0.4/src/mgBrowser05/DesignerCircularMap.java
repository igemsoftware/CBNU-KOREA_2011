...
public class DesignerCircularMap extends JComponent {
	...
	public DesignerCircularMap(DesignerGetInformation dGetInfo, MonitorPanel monitorPanel) {
	...
	void init() {
		...
		String sFile = dGetInfo.getFileName();
/*		if (!sFile.endsWith(".gb")) {
			for (i = 0; i < dGetInfo.getGeneNumber(); i++) {
				if (dGetInfo.getStrand(i)== -1) 
					gbColorList.add(new Color(255, 0, 0));     //적색
				else if (dGetInfo.getStrand(i)== +1) 
					gbColorList.add(new Color(0, 0, 255));     //청색
			}
		}else{
*/			
		String str ="";
		for (i = 0; i < dGetInfo.egFuncCatVec.size(); i++) {
			funcCat = dGetInfo.egFuncCatVec.elementAt(i).charAt(0);
			str = (String)dGetInfo.egFuncCatVec.elementAt(i);
								...
								for (int i = 0; i < generalpath.length; i++) {
									...
									if (generalpath[i].contains((double) e.getX(), 
										...
										monitorPanel.geneLabel.setText(
											"   Gene Name  : "+ dGetInfo.getGeneName(i));
										monitorPanel.geneStartLabel.setText(
											"   Gene Start : "+ dGetInfo.getSP(i));
										monitorPanel.geneEndLabel.setText(
											"   Gene End   : "+ dGetInfo.getEP(i));
									}
								}
							}
