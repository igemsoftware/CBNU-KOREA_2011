...
public class McFileCreator {                     // Making egBricks is easy!
	...
	public Vector<Integer> mcSPVec = new Vector<Integer>();       //MG에서의 new location, 디자인 환경에 따라 가변적인 자료임
	public Vector<Integer> mcEPVec = new Vector<Integer>();
	Vector<String> mcFuncCatVec = new Vector<String>();
	//Vector<Integer> mcIndexVec = new Vector<Integer>();
	Vector<Integer> mcLineNumVec = new Vector<Integer>();
	Vector<Integer> mcStrandVec = new Vector<Integer>();
...
//	static File tFile;
	...
	public McFileCreator(String sFile){
...
*/	
	...
	public Vector<Integer> getMcSPVec() { 
		return mgSPVec;
	}
	
	public Vector<Integer> getMcEPVec() {                                
		return mgEPVec;
	}
	...
	void correctSP() {
		...
		for (int i = 0; i < cdsIndexVec.size(); i++){        // cdsSP. tRNASP, rRNASP, ncRNASP와 gSP 의 차이 검정. end point 는 ?????
			int cdsSP = cdsSPVec.elementAt(i).intValue();
		...
		for (int i = 0; i < tRNAIndexVec.size(); i++){        // cdsSP. tRNASP, rRNASP, ncRNASP와 gSP 의 차이 검정.
		...
		for (int i = 0; i < rRNAIndexVec.size(); i++){        // cdsSP. tRNASP, rRNASP, ncRNASP와 gSP 의 차이 검정.
		...
		for (int i = 0; i < ncRNAIndexVec.size(); i++){        // cdsSP. tRNASP, rRNASP, ncRNASP와 gSP 의 차이 검정.
		...
		for (int i = 0; i < tmRNAIndexVec.size(); i++){        // cdsSP. tRNASP, rRNASP, ncRNASP와 gSP 의 차이 검정.
	...
	public String joinEGseq() throws IllegalSymbolException{        // 필수 유전자 개개의 서열을 연결, mcSeq 생성
		...
		Sequence seq = DNATools.createDNASequence(mcSeq, "mcSeq");
		
		System.out.println("mcSeq.length= " + seq.length() + ", " + "strLength= " + strLength);
	...
	public void makeEGVec(String sFile) {
	    ...
	    int sp;                           
	    for (int i = 0; i < csSPVec.size(); i++){       // 필수 유전자의 행 번호를 알기 위하여
	    	sp = csSPVec.elementAt(i).intValue();
			for (int j = 0; j < cSPVec.size(); j++){                 
				  if ((cSPVec.elementAt(j)).intValue() == sp) {
	    			  egLineNumVec.add(gLineNumVec.elementAt(i).intValue()); 
	    			  egIndexVec.add(i);
	    			  egSPVec.add(sp);
...
/*	
	public void makeMCVec(String sFile) {
		
	    for (int i = 0; i < egFuncCatVec.size(); i++){         // MC 유전자의 행 번호를 알기 위하여
	    	if ((egFuncCatVec.elementAt(i).toString() == "L") || (egFuncCatVec.elementAt(i).toString() == "D")) {
    			  mcLineNumVec.add(egLineNumVec.elementAt(i).intValue()); 
    			  mcIndexVec.add(egIndexVec.elementAt(i).intValue());
    			  mcSPVec.add(egSPVec.elementAt(i).intValue());
    			  mcEPVec.add(egEPVec.elementAt(i).intValue());
    			  mcStrandVec.add(egStrandVec.elementAt(i).intValue());
    			  mcFuncCatVec.add((String)egFuncCatVec.elementAt(i));
			}
	    }
		
		System.out.println("mcLineNum= " + mcLineNumVec.size() + ", " + "mcIndex= " + mcIndexVec.size() + ", " + "mcSP= " + mcSPVec.size() +
				", " + "mcEP= " + mcEPVec.size() + ", " + "mcFunCat= " + mcFuncCatVec.size());	
	}
*/		
	public void getEgFeature(String sFile) throws IllegalSymbolException {
	 ...
	 if (oSPVec.size() != 0) {
		try {  
			...
			String tFile = ".\\data\\mGenome\\mc_" + sFileName;
	 ...
	 }else System.out.println("oSPVec.size= " + oSPVec.size() + ", so " + orgName + " has no essential replication genes?");
	}
	...
	void makeEGInfoFile() {
		...
		try {
			...
			raf.writeBytes("LOCUS= " + richSeq.getName() + ", " + "GenomeSeqLen= " + richSeq.length());
			raf.writeBytes("\n");
			raf.writeBytes("gSP= " + gSPVec.size() + ", " + "csSP= " + csSPVec.size());
			raf.writeBytes("\n");
			int sum = cdsSPVec.size() + tRNASPVec.size() + rRNASPVec.size() + ncRNASPVec.size() + tmRNASPVec.size()
										+ miscRNASPVec.size();
			raf.writeBytes("cdsSP= " + cdsSPVec.size() + ", " +
					"tRNASP= " + tRNASPVec.size() + ", " + "rRNASP= " + rRNASPVec.size() +
					", " + "ncRNASP= " + ncRNASPVec.size() + ", " + "tmRNASP= " + tmRNASPVec.size() + 
					", " + "miscRNASP= " + miscRNASPVec.size() + ", " + "SUM= " + sum);
			raf.writeBytes("\n");
			
			int dif = csSPVec.size() - sum;
			raf.writeBytes("Coding Sequence - (CDS + tRNA + rRNA + ncRNA + tmRNA + misc_RNA) = " + dif);
			raf.writeBytes("pseudoLineNum= " + pseudoLineNumVec.size() + ", " + "pseudoIndex= " + pseudoIndexVec.size());
			raf.writeBytes("\n");
			raf.writeBytes("oSP= " + oSPVec.size() + ", " + "cSP= " + cSPVec.size() + ", " +
					"egSP= " + egSPVec.size() + ", " + "mgSP= " + mgSPVec.size() + 
					", " + "funcClass= " + funcClassVec.size() + ", " + "egFuncCat= " + egFuncCatVec.size());
			raf.writeBytes("\n");
			raf.writeBytes("gGeneName= " + gGeneNameVec.size() + ", " + "geneName= " + geneNameVec.size() + ", " + 
					"locusTag= " + locusTagVec.size() + ", " +
					"gStrand= " + gStrandVec.size());
			raf.writeBytes("\n");
			raf.writeBytes("meSP= " + meSPVec.size() + ", " +  "rrSP= " + rrSPVec.size() +
					", " + "mpSP= " + mpSPVec.size()  +
					", " + "rbsSP= " + rbsSPVec.size() + ", " + "pribnowSP= " + pribnowSPVec.size() + 
					", " + "recognitionSP= " + recognitionSPVec.size());
			raf.writeBytes("\n");
			raf.writeBytes("designerSeqLen= " + designerSeq.length());
			raf.writeBytes("\n" + "\n");
	...
	public static void main(String[] args) throws IllegalSymbolException {
		...
		System.out.println("gSPVec: " + mcFileCreator.gSPVec.size() + ", " + "csSPVec: " + mcFileCreator.csSPVec.size() + ", " + "cdsSPVec: " + mcFileCreator.cdsSPVec.size() + ", " +
				"tRNASPVec: " + mcFileCreator.tRNASPVec.size() +
				", " + "rRNASPVec: " + mcFileCreator.rRNASPVec.size() + ", " + "ncRNASPVec: " + mcFileCreator.ncRNASPVec.size() +
				", " + "tmRNASPVec: " + mcFileCreator.tmRNASPVec.size());
		
	
		
//		mcFileCreator.getFeatureIndex(sFile);
		System.out.println("cdsIndexVec: " + mcFileCreator.cdsIndexVec.size() + ", " + "pseudoLineNumVec: " + mcFileCreator.pseudoLineNumVec.size() + ", " + 
				"pseudoIndexVec: " + mcFileCreator.pseudoIndexVec.size() + ", " + "tRNAIndexVec: " + mcFileCreator.tRNAIndexVec.size() +
					", " + "rRNAIndexVec: " + mcFileCreator.rRNAIndexVec.size() + ", " + "ncRNAIndexVec: " + mcFileCreator.ncRNAIndexVec.size() +
					", " + "tmRNAIndexVec: " + mcFileCreator.tmRNAIndexVec.size());
		
//		mcFileCreator.deletePseudo(sFile);
		System.out.println("cdsIndexVec: " + mcFileCreator.cdsIndexVec.size() + ", " + "pseudoLineNumVec: " + mcFileCreator.pseudoLineNumVec.size() + ", " + 
				"tRNAIndexVec: " + mcFileCreator.tRNAIndexVec.size() +
					", " + "rRNAIndexVec: " + mcFileCreator.rRNAIndexVec.size() + ", " + "ncRNAIndexVec: " + mcFileCreator.ncRNAIndexVec.size() +
					", " + "tmRNAIndexVec: " + mcFileCreator.tmRNAIndexVec.size());

		
//		mcFileCreator.correctSP();
		System.out.println("csSPVec: " + mcFileCreator.csSPVec.size() + ", " + "cdsSPVec: " + mcFileCreator.cdsSPVec.size() +
				", " + "tRNASPVec: " + mcFileCreator.tRNASPVec.size() +
				", " + "rRNASPVec: " + mcFileCreator.rRNASPVec.size() + ", " + "ncRNASPVec: " + mcFileCreator.ncRNASPVec.size() +
				", " + "tmRNASPVec: " + mcFileCreator.tmRNASPVec.size());
		
		System.out.println("Uncorrected error found: ");
//		mcFileCreator.uncorrectedError();
/*
		mcFileCreator.getOrgName(sFile);
		mcFileCreator.makeBrickDB();
		
		mcFileCreator.getEgData();
		mcFileCreator.compareEgGenome(); 
		mcFileCreator.makeEGVec(sFile);
		
		mcFileCreator.getEgFeature(sFile);
		mcFileCreator.makeEGInfoFile();
*/		
		System.out.println("orgName= " + orgName);
		System.out.println("LOCUS= " + mcFileCreator.richSeq.getName() + ", " + "GenomeSeqLen= " + mcFileCreator.richSeq.length());
		System.out.println("gSPVec= " + mcFileCreator.gSPVec.size() + ", " + "csSPVec= " + mcFileCreator.csSPVec.size() + ", " +
				"cdsSPVec= " + mcFileCreator.cdsSPVec.size() + ", " +
				"tRNASPVec= " + mcFileCreator.tRNASPVec.size() + ", " + "rRNASPVec= " + mcFileCreator.rRNASPVec.size() +
				", " + "ncRNASPVec= " + mcFileCreator.ncRNASPVec.size() +
				", " + "tmRNASPVec= " + mcFileCreator.tmRNASPVec.size());
		System.out.println("pseudoLineNumVec= " + mcFileCreator.pseudoLineNumVec.size() + ", " + "pseudoIndexVec= " + mcFileCreator.pseudoIndexVec.size());
		System.out.println("oSPVec= " + mcFileCreator.oSPVec.size() + ", " + "cSPVec= " + mcFileCreator.cSPVec.size() + ", " +
				"egSPVec= " + mcFileCreator.egSPVec.size() + ", " + "mgSPVec= " + mcFileCreator.mgSPVec.size() + 
				", " + "funcClassVec= " + mcFileCreator.funcClassVec.size() + ", " + "egFuncCatVec= " + mcFileCreator.egFuncCatVec.size());
		System.out.println("gGeneNameVec= " + mcFileCreator.gGeneNameVec.size() + ", " + "geneNameVec= " + mcFileCreator.geneNameVec.size() + ", " + 
				"locusTagVec= " + mcFileCreator.locusTagVec.size() + ", " +
				"gStrandVec= " + mcFileCreator.gStrandVec.size());
		System.out.println("meSPVec= " + mcFileCreator.meSPVec.size() + ", " +  "rrSPVec= " + mcFileCreator.rrSPVec.size() +
				", " + "mpSPVec= " + mcFileCreator.mpSPVec.size()  +
				", " + "rbsSPVec= " + mcFileCreator.rbsSPVec.size() + ", " + "pribnowSPVec= " + mcFileCreator.pribnowSPVec.size() + 
				", " + "recognitionSPVec= " + mcFileCreator.recognitionSPVec.size());
