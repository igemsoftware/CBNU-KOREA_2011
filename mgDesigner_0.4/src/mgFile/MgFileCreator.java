...
public class MgFileCreator {                     // Making egBricks is easy!
	 ...
	 Vector<String> bgStrandVec = new Vector<String>();    //genome 상의 모든 유전자 방향 
	...
	Vector<String> egStrandVec = new Vector<String>();
...
//	static File tFile;
	...
	public MgFileCreator(String sFile){
		...
		if(richSeq.getCircular()) circular = "+";  
		...
		for (Iterator <Feature> fi = fh.features(); fi.hasNext();){      //Iterate through the CDS features 
		    ...
		    if (rf.getStrand().getValue() == + 1)      //Get the strand orientation of the feature .getToken()은 +, -로
				bgStrandVec.add("+");
			else if (rf.getStrand().getValue() == - 1)
				bgStrandVec.add("-");
		    
		    RichAnnotation ra = (RichAnnotation)rf.getAnnotation();      //Get the annotation of the feature
		    locusTag = (String) ra.getProperty("locus_tag");
	        locusTagVec.add(locusTag);
	        
//	        gi = (String) ra.getProperty("GI_tag");
//	        giVec.add(gi);
	        ...
	        if (!found) geneNameVec.add(locusTag); // 유전자의 gene name이 없는 경우는 대신 locus_tag을 사용
	        if (!found) geneName = "";
...
//		makeEGInfoFile();
	...
	public Vector<String> getLocusTagVec() {                 // 유전자 이름 목록 반환
		return locusTagVec;
	}
	
	public Vector<String> getgiVec() {                 // 유전자 이름 목록 반환
		return giVec;
	}
...
*/	
	...
	public Vector<String> getStrandVec() {                     // 유전자 방향      +, 1
		return bgStrandVec;
	}

	String getStrand(int i) {                     // 유전자 방향      +1, -1
		return (bgStrandVec.elementAt(i));
	}
	...
	public Vector<Integer> getCDSIndexVec() {                                
		return cdsIndexVec;
	}
	
	public Vector<Integer> gettRNAIndexVec() {                                
		return tRNAIndexVec;
	}
	
	public Vector<Integer> getrRNAIndexVec() {                                
		return rRNAIndexVec;
	}
	
	public Vector<Integer> getncRNAIndexVec() {                                
		return ncRNAIndexVec;
	}
	
	public Vector<Integer> gettmRNAIndexVec() {                                
		return tmRNAIndexVec;
	}
	
	public Vector<Integer> getmiscRNAIndexVec() {                                
		return miscRNAIndexVec;
	}
	
	public Vector<Integer> getPseudoIndexVec() {                                
		return pseudoIndexVec;
	}
	...
	void getFeatureIndex(String sFile){                                 // gene 시작점을 CS로 하지 않고 RBS로 한 경우
		...
		try {                                  
		    ...
		    while ((line = br.readLine()) != null) {       // 필수 유전자의 행 번호를 알기 위하여
				...
				if (line.startsWith("     CDS")) cdsIndexVec.add(gIdx-1);   //index 번호를 0 부터 하기위해
...
//					if (flag) System.out.println(egSPVec.elementAt(j).intValue());
	...
	void compareEgGenome(){                    //필수 유전자 정보와 비교하기 위하여
		...
		try {
			RandomAccessFile raf = new RandomAccessFile("dbError.txt", "rw");

			raf.seek(raf.length()); 	
			raf.writeBytes(orgName + ": oSP+ gSP-:");
			raf.writeBytes("\n");
			...
			for (int i = 0; i < oSPVec.size(); i++) {                         // oSP와 gSP 의 차이 검정. SP와 EP는 바뀔  수 있음. 따라서 SP와 함께 EP도 비교!
				...
				if (foundNum == 0) {                                              //SP에서 발견되지 않을 경우 EP 비교
					...
					if (!found) {				
						...
						raf.writeBytes("oSP= " + oSPVec.elementAt(i).intValue() + ", " + "oEP= " + oEPVec.elementAt(i).intValue());
						raf.writeBytes("\n");
					}
				}
			}
			//if (!duplicate) System.out.println("oSP에 중복된 것은 발견되지 않았습니다.");
			raf.writeBytes("oSPVec.size= " + oSPVec.size() + ",  " + "cSPVec.size= " + cSPVec.size());
			raf.writeBytes("\n");
	    ...
	    for (int i = 0; i < csSPVec.size(); i++){      //모든 유전자의  시작 행 번호를 벡터에 저장
			...
			for (int j = 0; j < cSPVec.size(); j++){            //cSPVec 의 유전자 순서를  gSPVec의 순서대로 수정하기 위하여                 
				  if ((cSPVec.elementAt(j)).intValue() == sp) {
	    			  ...
	    			  egStrandVec.add(bgStrandVec.elementAt(i).toString());
...
//		mgFileCreator.uncorrectedError();
		...
		System.out.println("gGeneNameVec= " + mgFileCreator.gGeneNameVec.size() + ", " + "geneNameVec= " + mgFileCreator.geneNameVec.size() + ", " + 
				...
				"gStrandVec= " + mgFileCreator.bgStrandVec.size());
